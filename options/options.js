      ...
      ${activeTab === 'general' && html`
        <div>
          ...
          <div class="section" id="section-engine">
            ...
            <div class="section-desc">Choose the AI service to summarize and extract metadata from captured chats.</div>
            ...
            ${settings.enrichEngine === 'claude' && html`
              <div>
                <div class="field">
                ...
                <div class="field">
                  <label class="field-label" for="endpoint-input">API Endpoint URL (Optional)</label>
                  <input
                    class="text-input"
                    type="text"
                    id="endpoint-input"
            ...
            ${settings.enrichEngine === 'gemini' && html`
              <div>
                <div class="field">
      ...
      ${activeTab === 'advanced' && html`
        <div>
          ...
          <div class="section" id="section-template">
            ...
            <div class="section-desc">Customize the markdown template used when injecting capsules. Supports: {title}, {summary}, {goals}, {stack}, {keyDecisions}, {constraints}.</div>
            <div class="field">
      ...
      ${activeTab === 'insights' && html`
        <div>
          ...
          <div class="section" id="section-stats">
            ...
            <!-- Platform Stats Grid -->
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 12px; margin-top: 15px; margin-bottom: 20px;">
              <div style="background: var(--bg-card); border: 1px solid var(--border-subtle); border-radius: var(--radius-sm); padding: 12px; text-align: center;">
                <div style="font-size: 11px; text-transform: uppercase; color: var(--text-secondary); margin-bottom: 4px;">Claude</div>
                <div style="font-size: 24px; font-weight: 700; color: var(--accent);">${stats.claude}</div>
              </div>
              <div style="background: var(--bg-card); border: 1px solid var(--border-subtle); border-radius: var(--radius-sm); padding: 12px; text-align: center;">
                <div style="font-size: 11px; text-transform: uppercase; color: var(--text-secondary); margin-bottom: 4px;">ChatGPT</div>
                <div style="font-size: 24px; font-weight: 700; color: var(--accent);">${stats.chatgpt}</div>
              </div>
              <div style="background: var(--bg-card); border: 1px solid var(--border-subtle); border-radius: var(--radius-sm); padding: 12px; text-align: center;">
                <div style="font-size: 11px; text-transform: uppercase; color: var(--text-secondary); margin-bottom: 4px;">Gemini</div>
                <div style="font-size: 24px; font-weight: 700; color: var(--accent);">${stats.gemini}</div>
              </div>
              <div style="background: var(--bg-card); border: 1px solid var(--border-subtle); border-radius: var(--radius-sm); padding: 12px; text-align: center;">
                <div style="font-size: 11px; text-transform: uppercase; color: var(--text-secondary); margin-bottom: 4px;">DeepSeek</div>
                <div style="font-size: 24px; font-weight: 700; color: var(--accent);">${stats.deepseek}</div>
              </div>
            </div>

            <!-- Top Tags Breakdown -->
            <div style="border-top: 1px solid var(--border-subtle); padding-top: 15px;">
              <div style="font-size: 14px; font-weight: 600; margin-bottom: 8px;">Top Extraction Tags</div>
              ${stats.sortedTags.length === 0 ? html`
                <div style="font-size: 12px; color: var(--text-secondary);">No tags captured yet. Try auto-tagging.</div>
              ` : stats.sortedTags.map(([tag, count]) => html`
                <div key=${tag} style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; font-size: 13px;">
                  <span style="background: var(--bg-input); padding: 3px 8px; border-radius: 4px; border: 1px solid var(--border-subtle);">${tag}</span>
                  <span style="color: var(--text-secondary); font-weight: 600;">${count} times</span>
                </div>
              `)}
            </div>
          </div>
